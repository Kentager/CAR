...
typedef enum {
  ...
  /*Other code settings are prohibited  ; When each mode is set, AK09911
     transits to the set mode.*/
...
typedef struct AK09911C_DATA {
  /*
          ...
          Note : Little endian
                two's complemnt         Hex	  	Dec
     Magnetic flux density [uT] 0000 1111 1111 1111    0FFF		4095
     1229(max.) 1111 0000 0000 0000    F000		-4096
     -1229(min.)
  */
  int16_t X_AXIS;
  int16_t Y_AXIS;
...
typedef struct AK09911C_4_QUADRANT_DATA_POINT_S {
  ...
  uint8_t first_quadant_data_pointok;
  uint8_t second_quadant_data_pointok;
  uint8_t third_quadant_data_pointok;
  uint8_t fourth_quadant_data_pointok;
...
/*
        ...
        HSM bit turns to ��1�� when I2C bus interface is changed from Standard
   or Fast mode to High-speed mode (Hs-mode).
        ---------------------------------
        bit 6 ~2  set  "0"
        ---------------------------------
   ...
   measurement mode 1, 2, 3, 4. It returns to ��0�� when any one of ST2 register
   or measurement data register (HXL to TMPS) is read.
        ...
        It returns to ��0�� when any one of ST2 register or measurement data
...
/*
        ...
        Measurement data is stored in two��s complement and Little Endian
   format. Measurement range of each axis is -8190 to 8190.
*/
...
/*
   ...
   regiseter is not saturated.

        In this case, measurement data is not correct and HOFL bit turns to
   ��1��. When next measurement start, it returns to ��0��. Refer to 6.4.3.6 for
   detailed information. ST2 register has a role as data reading end register,
   also. When any of measurement data register (HXL to TMPS) is read in
   Continuous measurement mode 1, 2, 3, 4,
        ****** it means data reading start and taken as data reading until ST2
   register is read.
        ****** Therefore, when any of measurement data is read, be sure to read
...
/*
   ...
   shipment. ASAX[7:0]:Magnetic sensor X-axis sensitivity adjustment value
*/
...
/*
        ...
        Sensitivity adjustment data for each axis is stored to fuse ROM on
   shipment. ASAY[7:0]:Magnetic sensor Y-axis sensitivity adjustment value
*/
...
/*
        ...
        Sensitivity adjustment data for each axis is stored to fuse ROM on
   shipment. ASAZ[7:0]:Magnetic sensor Z-axis sensitivity adjustment value
*/
...
// 外部变量声明（用于访问驱动内部的全局变量）
extern AK09911C5_ASA_DATA AK09911C_ASA_XYZ_DATA; // ASA 灵敏度补偿数据
...
/*
        ...
        AK09911C_CONTI_MODE_1=(0x01<<1),		->Continuous measurement
   mode 1 AK09911C_CONTI_MODE_2=(0x01<<2),		->Continuous measurement
   mode 2 AK09911C_CONTI_MODE_3=(0x03<<2),		->Continuous measurement
   mode 3 AK09911C_CONTI_MODE_4=(0x01<<3),		->Continuous measurement
   mode 4 AK09911C_SELF_TEST=(0x01<<4),			-> Self-test mode
        AK09911C_FUSE_ROM=(0x1F)				->Fuse ROM
   access mode
*/
...
/*
        ...
        use the final result save in
                ...
                first_quadrant_data[2][AK09911C_data_size]		->
   first_quadrant_data[0][AK09911C_data_size] as X-axis data ;
   first_quadrant_data[1][AK09911C_data_size] as Y-axis data
                second_quadrant_data[2][AK09911C_data_size]
                third_quadrant_data[2][AK09911C_data_size]
                fourth_quadrant_data[2][AK09911C_data_size]


        calculate the Xmax , Xmin , Ymax , Ymin ,Zmin ,Zmax find the
   Xoffset,Yoffset ,Zoffset. xoffset = (xmax + xmin)/2; yoffset = (ymax +
   ymin)/2; xgain =1; ygain = (final_x_max - final_x_min) / (final_y_max -
   final_y_min);
...
/* the pitch & roll angle data from other 3-Axis Accelerometer. example : KXSD9
 * , LIS3DH , ADXL345 , MMA8452 , LSM303DLHC..etc*/
