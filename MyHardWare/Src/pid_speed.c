...
static void update_motor_output(Speed_PID_Controller_t *controller,
                                float output);
static void pid_init(IncrementalPID_State_t *pid, float target, float kp,
                     float ki, float kd);
...
void Speed_PID_Update(Speed_PID_Controller_t *controller) {
  ...
  controller->speed_buffer_index =
      (controller->speed_buffer_index + 1) % SPEED_PID_BUFFER_SIZE;
