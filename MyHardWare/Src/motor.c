...
#else
// 双驱模式电机实例
static Motor_Class_t Motor_Left = {0, MOTOR_DIR_STOP, MOTOR_LEFT,
                                   MOTOR_FL_PWM_TIM, MOTOR_FL_PWM_CHANNEL};
static Motor_Class_t Motor_Right = {0, MOTOR_DIR_STOP, MOTOR_RIGHT,
                                    MOTOR_FR_PWM_TIM, MOTOR_FR_PWM_CHANNEL};
#endif
...
/**
 * @brief 通用电机速度设置函数
 * @param motor 电机 ID (MOTOR_FR/MOTOR_FL/MOTOR_BR/MOTOR_BL 或
 * MOTOR_RIGHT/MOTOR_LEFT)
 * @param speed 速度值 (-MOTOR_PWM_MAX_DUTY ~ +MOTOR_PWM_MAX_DUTY)
 *              正值：正转，负值：反转H，0 ：停止
 *
 * @note 此函数仅修改电机实例的 _speed 成员变量，不直接操作硬件,
 * speed最好不要传入负值，传入负值理论上就要去改motor的direction变量，暂且还没有写
 */
 ...
 * @param motor 电机 ID
 * @param dir 方向
 * (MOTOR_DIR_FORWARD/MOTOR_DIR_BACKWARD/MOTOR_DIR_STOP/MOTOR_DIR_BRAKE)
 *
...
void Motor_Update(Motor_Id_e motor) {
  ...
  uint16_t duty =
      (inst->_speed >= 0) ? (uint16_t)inst->_speed : (uint16_t)(-inst->_speed);
...
static void Motor_GPIO_Init(void) {
  ...
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(MOTOR_FL_PWM_PORT, &GPIO_InitStructure);
  GPIO_PinAFConfig(MOTOR_FL_PWM_PORT, GPIO_PinSource13, MOTOR_FL_PWM_AF);
...
void Motor_Driver_Init(void) {
  ...
  Motor_TIM_Init(MOTOR_FL_PWM_TIM, MOTOR_FL_PWM_TIM_CLK);
  Motor_TIM_Channel_Init(MOTOR_FL_PWM_TIM, MOTOR_FL_PWM_CHANNEL);
